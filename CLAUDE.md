...
**GraphQL API.** `apps/api/src/graphql.ts` is a code-first `graphql` schema mounted at `POST /api/graphql` (and GET for exploration), authenticated by the same cookie/Bearer identity as REST. Each request builds a read snapshot of the store into id-maps (`graphqlContext`) so nested field resolution is lazy Map lookups with no N+1; mutations (`createIssue`, `updateIssue`, `deleteIssue`, `createComment`) delegate to domain services. It's another transport adapter over the same `Domain`, like REST/MCP — not a separate data path.

**PWA.** `apps/web/public/` holds the web app manifest (`manifest.webmanifest`), icons (`icon.svg` + generated PNGs), and a hand-rolled service worker (`sw.js` — network-first navigations with an offline shell fallback, cache-first for hashed `/assets/`, API traffic never cached; no Workbox). `main.tsx` registers it; nginx serves `sw.js` with `no-cache` and the manifest as `application/manifest+json`. Vite copies `public/` into `dist/`.

**Monitor & AI.** **Custom dashboards** are a synced `Dashboard` entity (shared||creator, like `CustomView`) of composable tiles rendered client-side from the store by `apps/web/src/components/tiles.tsx` (extracted from the Insights page so both reuse them). **Pulse** (`PulseService`, `GET /api/pulse`) computes a cross-workspace activity digest on demand from stored entities — no new storage. **BYO-key AI** is optional and admin-set: `AiService` stores workspace LLM config in a non-synced `ai_settings` singleton (the API key never leaves the server; clients see only `AiSettingsPublic`), and `apps/api/src/llm.ts` is the provider adapter (Anthropic Messages / OpenAI Chat, like the other transport adapters). AI features (`apps/api/src/ai.ts`): a Pulse "Summarize" action and issue label suggestions, both gated on `domain.ai.isReady()`.
