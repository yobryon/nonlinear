...
| `get_issue`            | One issue with its comments and dependency edges (`blockedBy`/`blocks`) — `{ identifier }` e.g. `AUGRID-42`. Mature issues get big: `descriptionOnly` returns just the brief (no comments); `commentsAfter` (a comment id or ISO timestamp) returns only comments past that cursor, and the reply carries `latestCommentAt` to page from. |
...
| `reconcile_summary`    | `{ teamKey, staleDays? }` → board truth, each count beside its falsifier: `open`; `untouched` (stale N+ days); `inProgress` and `stalledUnowned` (in progress, nobody named as blocker — staleness-independent); `blockedOnUnresolved` (blocked by an issue that isn't done). A status-report line.                                       |
| `list_decisions`       | A team's decisions `{ teamKey, status?, since?, q?, limit?, cursor? }`, newest first. Paginated — returns `{ decisions, nextCursor }` (compact rows, no body; pass `nextCursor` back as `cursor`). The ledger only grows, so read it in pages.                                                                                            |
...
| Tool                    | Params                                                                                                                                                                                                                                                                                                               |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `create_issue`          | `{ teamKey, title, description?, priority?, assignee?, state?, labels?, project?, blockedBy?, blocks? }`. `blockedBy`/`blocks` take issue identifiers — a dependency on another **issue** (distinct from `waiting_on`, which names a **person**).                                                                    |
| `update_issue`          | `{ identifier, title?, description?, state?, priority?, assignee?, waiting_on?, project?, blockedBy?, blocks? }`. `blockedBy`/`blocks` are additive issue-dependency edges (idempotent); `waiting_on` is the person whose move unblocks it.                                                                          |
...
| `leave_issue`           | `{ identifier }` — remove **yourself** as subscriber (and unassign if you're the assignee) from an issue you can read but not edit. Self-removal needs no edit rights; silences a mis-subscription (e.g. a stale cross-team assignment) and clears its inbox items.                                                  |
