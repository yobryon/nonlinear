...
tells you _how the thing is built_, these documents tell you _what we decided to
build and why_. They are written for a new product or engineering teammate who
...
_reflected from the running app_ — every claim is grounded in code that exists
...
- **`CLAUDE.md`** — the build/architecture guide. It is the authoritative source
  ...
  design doc needs to explain _why_ a mechanism exists, it links back to the code
...
and **09** whenever you hit a "why on earth is it done _this_ way" moment — the
